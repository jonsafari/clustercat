...
float class_ngram_prob(const struct cmd_args cmd_args, const count_arrays_t count_arrays, struct_map_class *class_map[const], const sentlen_t i, const wclass_t class_i, const wclass_count_t class_i_count, wclass_t sent[const], const unsigned char ngram_order, const struct_model_metadata model_metadata, const float weights[const]) { // Cf. increment_ngram()
	...
	if (0 && ngram_order > cmd_args.max_array) {
		...
		for (; j >= 0; j--, max_ngram_used++) { // Traverse shortest string first && short-circuit if history not found
			...
			wclass_count_t numerator = map_find_count_fixed_width(class_map, &ngram[j]);
			...
			wclass_count_t denominator = map_find_count_fixed_width(class_map, &ngram[j]);
...
float ngram_prob(struct_map_word *ngram_map[const], const sentlen_t i, const char * restrict word_i, const word_count_t word_i_count, const struct_model_metadata model_metadata, char * restrict sent[const], const short word_lengths[const], const unsigned char ngram_order, const float weights[const]) { // Cf. increment_ngram()
	...
	for (j = start_position; j <= i; ++j, --history_len) { // Traverse longest n-gram string.  TODO: traverse shortest string first && short-circuit if history not found
		...
		if (history_len > 0) { // Bigrams and longer
			...
			word_count_t denominator_count = map_find_count(ngram_map, jp);
