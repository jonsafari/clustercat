...
typedef struct {
	char key[KEYLEN];
	char class[CLASSLEN]; // could be integer-type, but requires lots of restructuring
	UT_hash_handle hh;	// makes this structure hashable
...
void map_add_class(struct_map_word_class **map, const char * restrict entry_key, const char * restrict entry_class);

void map_update_class(struct_map_word_class **map, const char * restrict entry_key, const char * restrict entry_class);
...
char *get_class(struct_map_word_class *map[const], const char * restrict entry_key, char * restrict unk);
