...
struct cmd_args {
	...
	unsigned char   class_order : 4;
	unsigned char   max_array : 2;
	unsigned short  min_count : 9;
	unsigned short  num_threads : 8;
	char            verbose : 4;      // Negative values increasingly suppress normal output
	unsigned char   class_algo : 2;   // enum class_algos
	unsigned char   rev_alternate: 2; // How often to alternate using reverse pex.  0 == never, 1 == after every one normal pex cycles, ...
};
